...
export const demoBooks: Book[] = bookSeeds.map(
  ([title, author, lexile_level, genre, tags, accent], index) => ({
    id: index + 1,
    title,
    author,
    lexile_level,
    ...
    tags,
    accent,
    cover: ["#24483d", "#6d4d7d", "#266172", "#9b6a31", "#38457b", "#557347"][
      index % 6
    ],
  }),
);
...
function sessionsFor(
  id: number,
  risk: StudentRisk["risk_level"],
): ReadingSession[] {
  const sessions: ReadingSession[] = [];
  const slope = risk === "at_risk" ? -0.28 : risk === "watch" ? -0.02 : 0.24;
  for (let i = 0; i < 12; i += 1) {
    const wpm = Math.round(
      86 + id * 0.7 + slope * i * 7 + Math.sin(id + i) * 4,
    );
    const comprehension = Math.round(
      76 + (id % 7) + slope * i * 2 + Math.cos(id + i) * 3,
    );
    sessions.push({
...
export const demoStudents: StudentRisk[] = Array.from(
  { length: 24 },
  (_, index) => {
    const id = index + 1;
      ...
      wpm_slope:
        risk === "at_risk"
          ...
          : risk === "watch"
            ...
            : 0.16 + (index % 3) * 0.03,
      comprehension_slope:
        risk === "at_risk" ? -0.09 : risk === "watch" ? -0.01 : 0.07,
      avg_wpm: Math.round(
        sessions.reduce((sum, item) => sum + item.wpm, 0) / sessions.length,
      ),
      avg_comprehension: Math.round(
        sessions.reduce((sum, item) => sum + item.comprehension_score, 0) /
          sessions.length,
      ),
      session_count: 38,
...
async function loadCsvDemo() {
  const [studentsText, classroomsText, booksText, sessionsText] =
    await Promise.all([
  ...
  const classrooms = new Map(
    parseCsv(classroomsText).map((row) => [Number(row.id), row]),
  );
  ...
  parseCsv(sessionsText).forEach((row) => {
    const session = {
      ...
      wpm: Number(row.wpm),
      comprehension_score: Number(row.comprehension_score),
      minutes_read: Number(row.minutes_read),
      book_id: Number(row.book_id),
    };
    sessionsByStudent.set(Number(row.student_id), [
      ...(sessionsByStudent.get(Number(row.student_id)) ?? []),
      session,
    ]);
  });
  const books = parseCsv(booksText).map((row, index) => ({
    id: Number(row.id),
    title: row.title,
    author: row.author,
    lexile_level: Number(row.lexile_level),
    genre: row.genre,
    ...
    accent: ["#d7e8d3", "#e9d7ef", "#c9e4ed", "#f5dfb5"][index % 4],
  }));
  const students = parseCsv(studentsText).map((row) => {
    const id = Number(row.id);
    const classroom = classrooms.get(Number(row.classroom_id));
    const sessions = (sessionsByStudent.get(id) ?? []).sort((a, b) =>
      a.session_date.localeCompare(b.session_date),
    );
    const wpm_slope = regression(sessions, "wpm");
    const comprehension_slope = regression(sessions, "comprehension_score");
    const risk_level =
      wpm_slope < -0.05 ? "at_risk" : wpm_slope < 0.02 ? "watch" : "on_track";
    return {
      id,
      student_id: id,
      name: row.name,
      baseline_lexile: Number(row.baseline_lexile),
      interests: jsonArray(row.interests),
      grade: Number(classroom?.grade ?? 4),
      classroom_name: classroom?.name ?? "Room 104",
      wpm_slope,
      comprehension_slope,
      avg_wpm:
        sessions.reduce((sum, item) => sum + item.wpm, 0) /
        Math.max(sessions.length, 1),
      avg_comprehension:
        sessions.reduce((sum, item) => sum + item.comprehension_score, 0) /
        Math.max(sessions.length, 1),
      session_count: sessions.length,
      last_session: sessions[sessions.length - 1]?.session_date ?? "",
      risk_level: risk_level as StudentRisk["risk_level"],
    };
  });
...
export function getDemoDetail(id: number): StudentDetail {
  const student =
    demoStudents.find((item) => item.id === id) ?? demoStudents[0];
  return {
    student,
    trend: student,
    sessions: sessionsFor(student.id, student.risk_level),
  };
}
...
export function getDemoInsight(student: StudentRisk): Insight {
  const book =
    demoBooks.find(
      (item) => item.lexile_level >= student.baseline_lexile - 80,
    ) ?? demoBooks[0];
...
function normalizeBook(
  book: Omit<Book, "cover" | "accent"> &
    Partial<Pick<Book, "cover" | "accent">>,
) {
  return {
    ...book,
    cover: book.cover ?? coverPalette[book.id % coverPalette.length],
    accent: book.accent ?? "#d7e8d3",
  };
}
...
export async function loadStudents(): Promise<{
  ...
  try {
    ...
    /* Demo mode keeps the presentation usable without local services. */
  }
  try {
    return { students: (await getCsvDemo()).students, demo: true };
  } catch {
    return { students: demoStudents, demo: true };
  }
}

export async function loadStudentDetail(
  id: number,
): Promise<{ detail: StudentDetail; demo: boolean }> {
  try {
    return {
      detail: await fetchJson<StudentDetail>(`/api/students/${id}`),
      demo: false,
    };
  } catch {
    try {
      ...
      const student =
        csv.students.find((item) => item.id === id) ?? csv.students[0];
      return {
        detail: {
          student,
          trend: student,
          sessions: csv.sessionsByStudent.get(student.id) ?? [],
        },
        demo: true,
      };
    } catch {
      return { detail: getDemoDetail(id), demo: true };
    }
  }
}
...
export async function loadClassPulse(
  ...
  const details = await Promise.all(
    students.map((student) => loadStudentDetail(student.id)),
  );
  const byDate = new Map<string, ReadingSession[]>();
  details.forEach(({ detail }) =>
    detail.sessions.forEach((session) => {
      byDate.set(session.session_date, [
        ...(byDate.get(session.session_date) ?? []),
        session,
      ]);
    }),
  );
  return Array.from(byDate.entries())
    ...
    .map(([session_date, sessions]) => ({
      session_date,
      wpm: Math.round(
        sessions.reduce((sum, session) => sum + session.wpm, 0) /
          sessions.length,
      ),
      comprehension_score: Math.round(
        sessions.reduce(
          (sum, session) => sum + session.comprehension_score,
          0,
        ) / sessions.length,
      ),
      minutes_read: Math.round(
        sessions.reduce((sum, session) => sum + session.minutes_read, 0) /
          sessions.length,
      ),
      book_id: sessions[0].book_id,
    }));
}

export async function loadBooks(): Promise<Book[]> {
  try {
    const books = await fetchJson<Array<Omit<Book, "cover" | "accent">>>(
      "/api/students/books",
    );
    if (books.length) return books.map(normalizeBook);
  } catch {
    /* Fall back to the bundled catalog when the API is unavailable. */
  }
  try {
    return (await getCsvDemo()).books;
  } catch {
    return demoBooks;
  }
}
...
export async function loadInsight(
  id: number,
  student: StudentRisk,
): Promise<{ insight: Insight; demo: boolean }> {
  try {
    const insight = await fetchJson<Insight>(`/api/students/${id}/insight`);
    return {
      insight: {
        ...insight,
        recommended_book: insight.recommended_book
          ? normalizeBook(insight.recommended_book)
          : null,
      },
      demo: false,
    };
